	...
	fx::ServerInstanceBase::OnServerCreate.Connect([](fx::ServerInstanceBase* instance)
	{
		if (!licenseChecked)
		{
			...
			if (!var->GetValue().empty())
			{
				...
				opts.ipv4 = true;
				httpClient->DoPostRequest(fmt::sprintf(LICENSING_EP "api/server/register"), jsonData.dump(), opts, [instance, tlm](bool success, const char* data, size_t length)
				{
					if (!success)
					{
						FatalError("A connection with the IRFive server could not be established!");
					}
					else
					{
